<template>
  <div class="my-layout my-container">
    ...
    <base-purpose-form :purposeTypeEnum="props.purposeTypeEnum" ref="basePurposeFormRef" :title="state.basePurposeFormTitle"></base-purpose-form>
  </div>
</template>
...
// 引入组件
const BasePurposeForm = defineAsyncComponent(() => import('./base-purpose-form.vue'))
...
onMounted(async () => {
  await new BaseGroupApi()
    .getAll()
    .then((res) => {
      if (props.purposeTypeEnum == PurposeTypeEnum.Pathology) {
        state.search[0].options = [{ label: '病理', value: '9999' }]
      } else {
...
const onQuery = async () => {
  state.loading = true
  if (props.purposeTypeEnum == PurposeTypeEnum.Pathology) {
    state.pageInput.filter!.groupCode = '9999'
    state.pageInput.filter!.containsPathology = true
  }
