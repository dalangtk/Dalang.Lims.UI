...
const state = reactive({
  showDialog: false,
  sureLoading: false,
  form: { isValid: true, opened: true } as PermissionUpdateGroupInput,
})
...
const open = async (row: any = {}) => {
  ...
  if (row.id > 0) {
    ...
    state.form = { isValid: true, opened: true, icon: 'ele-Memo', parentId: row.parentId } as PermissionUpdateGroupInput
  }
  proxy.$modal.closeLoading()
