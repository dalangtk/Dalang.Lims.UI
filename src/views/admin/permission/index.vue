          ...
          >
            ...
            <template #dropdown>
              <el-dropdown-menu>
                ...
                <el-dropdown-item v-if="auth('api:admin:permission:adddot')" @click="onAdd({ type: 4 })">新增权限点(Api)</el-dropdown-item>
              </el-dropdown-menu>
            </template>
          </el-dropdown>
        </el-form-item>
    ...
    <el-card class="my-fill mt8" shadow="never">
      ...
      >
        ...
        <el-table-column prop="type" label="类型" width="80" show-overflow-tooltip>
          <template #default="{ row }">
            {{ row.type === 1 ? '分组' : row.type === 2 ? '菜单' : row.type === 3 ? '权限点' : row.type === 4 ? 'Api' : '' }}
          </template>
        </el-table-column>
        <el-table-column prop="path" label="权限地址" min-width="240" show-overflow-tooltip>
          <template #default="{ row }">
            ...
            <div v-if="row.type === 3 || row.type === 4">接口地址：{{ row.apiPaths }}</div>
          </template>
        </el-table-column>
          ...
          <template #default="{ row }">
            <el-button
              v-if="
                ...
                ((row.type === 3 || row.type === 4) && auth('api:admin:permission:updatedot'))
              "
              icon="ele-EditPen"
              size="small"
              text
            ...
            >
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item v-if="row.type === 2 && auth('api:admin:permission:addmenu')" @click="onAdd({ type: 4, parentId: row.id })">
                    新增权限点(Api)
                  </el-dropdown-item>
...
const PermissionApiForm = defineAsyncComponent(() => import('./components/permission-api-form.vue'))
...
const permissionApiFormRef = ref()
...
const onAdd = (row: PermissionListOutput) => {
  switch (row.type) {
    ...
    case 3:
      ...
      break
    case 4:
      state.permissionFormTitle = '新增权限点(Api)'
      permissionApiFormRef.value.open({ parentId: row.parentId })
      break
  }
}
...
const onEdit = (row: PermissionListOutput) => {
  switch (row.type) {
    ...
    case 3:
      ...
      break
    case 4:
      state.permissionFormTitle = '编辑权限点(Api)'
      permissionApiFormRef.value.open(row)
      break
  }
}
...
const onDelete = (row: PermissionListOutput) => {
  proxy.$modal
    .confirmDelete(
      `确定要删除${row.type === 1 ? '分组' : row.type === 2 ? '菜单' : row.type === 3 ? '权限点' : row.type === 4 ? 'Api' : ''}【${row.label}】?`
    )
    .then(async () => {
