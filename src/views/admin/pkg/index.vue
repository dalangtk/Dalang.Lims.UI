  ...
  <my-layout>
    <pane size="50" min-size="30" max-size="70">
      <div class="my-flex-column w100 h100">
        ...
        <el-card class="my-fill mt8" shadow="never">
          <el-table ref="pkgTableRef" v-loading="state.loading" :data="state.pkgData" default-expand-all
            highlight-current-row style="width: 100%" @current-change="onTableCurrentChange">
            ...
            <el-table-column label="操作" width="100" fixed="right" header-align="center" align="center">
              <template #default="{ row }">
                <my-dropdown-more
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item v-if="auth('api:admin:pkg:set-pkg-permissions')"
                        @click="onSetPkgMenu(row)">菜单权限</el-dropdown-item>
                      <el-dropdown-item v-if="auth('api:admin:pkg:update')" @click="onEdit(row)">编辑套餐</el-dropdown-item>
                      <el-dropdown-item v-if="auth('api:admin:pkg:delete')"
                        @click="onDelete(row)">删除套餐</el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
                </my-dropdown-more>
              </template>
          ...
          <div class="my-flex my-flex-end" style="margin-top: 20px">
            <el-pagination v-model:currentPage="state.pageInput.currentPage"
              v-model:page-size="state.pageInput.pageSize" :total="state.total" :page-sizes="[10, 20, 50, 100]"
              size="small" background @size-change="onSizeChange" @current-change="onCurrentChange"
    ...
    <pane>
      <div class="my-flex-column w100 h100">
        <el-card class="mt8" shadow="never" :body-style="{ paddingBottom: '0' }">
          <el-form :inline="true" @submit.stop.prevent>
            ...
            <el-form-item>
              ...
              <el-button v-auth="'api:admin:pkg:add-pkg-tenant'" type="primary" icon="ele-Plus" @click="onAddTenant">
                添加企业
              </el-button>
              <el-button v-auth="'api:admin:pkg:remove-pkg-tenant'" type="danger" icon="ele-Delete"
        ...
        <el-card class="my-fill mt8" shadow="never">
          <el-table ref="tenantTableRef" v-loading="state.tenantListLoading" :data="state.tenantData" row-key="id"
            style="width: 100%" @row-click="onTenantRowClick">
          ...
          <div class="my-flex my-flex-end" style="margin-top: 20px">
            <el-pagination v-model:currentPage="state.tenantPageInput.currentPage"
              v-model:page-size="state.tenantPageInput.pageSize" :total="state.tenantTotal"
              :page-sizes="[10, 20, 50, 100]" size="small" background @size-change="onTenantSizeChange"
              @current-change="onTenantCurrentChange" layout="total, sizes, prev, pager, next, jumper" />
          </div>
        </el-card>
      </div>
    </pane>
    ...
    <tenant-select ref="tenantSelectRef" :title="`添加【${state.pkgName}】企业`" multiple :sure-loading="state.sureLoading"
      @sure="onSureTenant"></tenant-select>
