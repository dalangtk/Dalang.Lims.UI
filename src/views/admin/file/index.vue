  ...
  <div class="my-layout">
    ...
    <el-card class="my-fill mt8" shadow="never">
      <el-table v-loading="state.loading" :data="state.fileListData" row-key="id" style="width: 100%">
        ...
        <el-table-column prop="proName" label="上传者" width="80">
          <template #default="{ row }">
            {{ row.modName || row.proName || '' }}
          </template>
        </el-table-column>
        <el-table-column prop="proTime" label="更新时间" width="100">
          <template #default="{ row }">
            {{ formatterTime(row.modTime || row.proTime || '') }}
          </template>
        </el-table-column>
