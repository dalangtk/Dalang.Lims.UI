  ...
  <my-layout>
    <pane size="20" min-size="20" max-size="35">
      <div class="my-flex-column w100 h100">
        <org-menu @node-click="onOrgNodeClick" :modelValue="state.modelValue" select-first-node></org-menu>
      </div>
    </pane>
    <pane size="80">
      <div class="my-flex-column w100 h100">
            ...
            <el-table-column label="操作" width="140" header-align="center" align="center" fixed="right">
              <template #default="{ row }">
                ...
                >
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item v-if="auth('api:lims:base-user-group:update')" @click="onSetUserGroup(row)">组别权限</el-dropdown-item>
      ...
      <UserGroupDialog ref="userGroupRef"></UserGroupDialog>
    </pane>
  </my-layout>
</template>
...
import UserGroupDialog from '/@/views/lims/basedata/baseusergroup/components/usergroupdialog.vue'
...
const onSetUserGroup = (row: UserGetPageOutput) => {
  console.log(row)
  userGroupRef.value.open(row)
}
//设置或取消主管
    ...
    proxy.$modal
      ...
      .then(async () => {
        ...
        const res = await new UserApi()
          .setEnable({ userId: row.id, isValid: !row.isValid }, { showSuccessMessage: true })
          .catch(() => {
