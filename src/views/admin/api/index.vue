<template>
  <div class="my-layout">
    ...
    <el-card class="my-fill mt8" shadow="never">
      <el-table :data="state.apiTreeData" style="width: 100%" v-loading="state.loading" row-key="id"
        :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" :expand-row-keys="state.expandRowKeys">
        ...
        <el-table-column label="操作" width="160" fixed="right" header-align="center" align="center">
          <template #default="{ row }">
            <el-button v-auth="'api:admin:api:update'" icon="ele-EditPen" size="small" text type="primary"
              @click="onEdit(row)">编辑</el-button>
            <el-button v-auth="'api:admin:api:delete'" icon="ele-Delete" size="small" text type="danger"
...
const onSync = async () => {
  ...
  swaggerResourceUrls.forEach(async (swaggerResourceUrl, swaggerResourcesIndex) => {
    console.log('swaggerResourceUrl', swaggerResourceUrl);
    const resSwaggerResources = await new ApiExtApi().getSwaggerResources(swaggerResourceUrl, { showErrorMessage: false }).catch(() => {
      state.syncLoading = false
    })
    console.log('resSwaggerResources' + swaggerResourceUrl, resSwaggerResources)
