    ...
    <el-card class="my-fill mt8" shadow="never">
      <el-table v-loading="state.loading" :data="state.tenantListData" row-key="id" height="'100%'"
        ...
        <el-table-column label="状态" width="80" align="center" fixed="right">
          <template #default="{ row }">
            <el-switch v-if="auth('api:admin:tenant:set-enable')" v-model="row.isValid" :loading="row.loading"
        ...
        <el-table-column label="操作" width="140" header-align="center" align="center" fixed="right">
          <template #default="{ row }">
            ...
            <my-dropdown-more v-auths="['api:admin:tenant:delete']">
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item v-if="auth('api:admin:tenant:delete')"
                    @click="onDelete(row)">删除租户</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </my-dropdown-more>
          </template>
    ...
    proxy.$modal
      ...
      .then(async () => {
        ...
        const res = await new TenantApi()
          .setEnable({ tenantId: row.id, isValid: !row.isValid }, { showSuccessMessage: true })
          .catch(() => {
