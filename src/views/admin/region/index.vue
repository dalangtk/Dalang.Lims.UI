<template>
  <div class="my-layout">
    <el-card class="mt8" shadow="never" :body-style="{ paddingBottom: '0' }">
      <el-form :inline="true" @submit.stop.prevent>
        ...
        <el-form-item label="类型">
          <el-select v-model="state.filter.level" :empty-values="[null, undefined]" :value-on-clear="null"
            style="width: 100px" @change="onQuery">
            <el-option label="全部" :value="''" />
            <el-option v-for="item in state.regionLevelList" :key="item.label" :label="item.label"
              :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="状态">
          <el-select v-model="state.filter.isValid" :empty-values="[null, undefined]" :value-on-clear="null"
            style="width: 100px" @change="onQuery">
            <el-option v-for="item in state.statusList" :key="item.name" :label="item.name" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="热门">
          <el-select v-model="state.filter.hot" :empty-values="[null, undefined]" :value-on-clear="null"
            style="width: 100px" @change="onQuery">
        ...
        <el-form-item>
          ...
          <el-button v-if="auth('api:admin:region:sync-data')" ref="syncRef" :loading="state.sync.loading"
            type="primary" icon="ele-Refresh">
            同步
          </el-button>
          <el-popover v-if="auth('api:admin:region:sync-data')" ref="popoverRef" :virtual-ref="syncRef" trigger="click"
            virtual-triggering :width="300">
            <p class="my-flex my-flex-items-center">
              确定要同步地区至
              <el-select v-model="state.sync.regionLevel" size="small" :teleported="false"
    ...
    <el-card class="my-fill mt8" shadow="never">
      <el-table v-loading="state.loading" :data="state.dataList" default-expand-all highlight-current-row
        style="width: 100%">
        ...
        <el-table-column label="状态" width="80" align="center" fixed="right">
          <template #default="{ row }">
            <el-switch v-if="auth('api:admin:region:set-enable')" v-model="row.isValid" :loading="row.loading"
              :active-value="true" :inactive-value="false" inline-prompt active-text="启用" inactive-text="禁用"
              :before-change="() => onSetEnable(row)" />
            <template v-else>
              <el-tag type="success" v-if="row.isValid">启用</el-tag>
        ...
        <el-table-column label="热门" width="80" align="center" fixed="right">
          <template #default="{ row }">
            <el-switch v-if="auth('api:admin:region:set-hot')" v-model="row.hot" :loading="row.hotLoading"
              :active-value="true" :inactive-value="false" inline-prompt active-text="是" inactive-text="否"
              :before-change="() => onSetHot(row)" />
            <template v-else>
              <el-tag type="success" v-if="row.isValid">是</el-tag>
        ...
        <el-table-column label="操作" width="160" fixed="right" header-align="center" align="center">
          <template #default="{ row }">
            <el-button v-auth="'api:admin:region:update'" icon="ele-EditPen" size="small" text type="primary"
              @click="onEdit(row)">编辑</el-button>
            <el-button v-auth="'api:admin:region:delete'" icon="ele-Delete" size="small" text type="danger"
      ...
      <div class="my-flex my-flex-end" style="margin-top: 20px">
        <el-pagination v-model:currentPage="state.pageInput.currentPage" v-model:page-size="state.pageInput.pageSize"
          :total="state.total" :page-sizes="[10, 20, 50, 100]" size="small" background @size-change="onSizeChange"
...
const onSetEnable = (row: RegionGetPageOutput & { loading: boolean }) => {
  return new Promise((resolve, reject) => {
    proxy.$modal
      .confirm(`确定要${row.isValid ? '禁用' : '启用'}【${row.name}】?`)
      .then(async () => {
        row.loading = true
        const res = await new RegionApi()
          .setEnable({ regionId: row.id, isValid: !row.isValid }, { showSuccessMessage: true })
          .catch(() => {
