<template>
  <div class="my-layout">
    <el-card class="mt8" shadow="never" :body-style="{ paddingBottom: '0' }">
      <el-form :model="state.filterModel" :inline="true" @submit.stop.prevent>
        <el-form-item prop="name">
          <el-input v-model="state.filterModel.proName" placeholder="操作账号" @keyup.enter="onQuery" />
        </el-form-item>
        <el-form-item>
    ...
    <el-card class="my-fill mt8" shadow="never">
      <el-table ref="tableRef" v-loading="state.loading" :data="state.oprationLogListData" row-key="id"
        style="width: 100%">
        <el-table-column prop="proName" label="操作账号" width="100">
          <template #default="{ row }"> {{ row.proName }}<br />{{ row.nickName }} </template>
        </el-table-column>
        ...
        <el-table-column prop="status" label="操作状态" width="80">
          <template #default="{ row }">
            <el-tag :type="row.status ? 'success' : 'danger'" disable-transitions>{{ row.status ? '成功' : '失败'
              }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="proTime" label="操作时间" :formatter="formatterTime" width="160" />
