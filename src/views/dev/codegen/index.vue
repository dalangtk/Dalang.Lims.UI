<template>
  <div style="height: 100%; display: flex; flex-direction: column">
        ...
        <el-col :span="18">
          <el-form inline :model="state.filter">
            ...
            <el-form-item label="表名" class="search-box-item">
              <el-input clearable v-model="state.filter.tableName" placeholder="模糊搜索" @keyup.enter="getConfigs"> </el-input>
            </el-form-item>
            <el-form-item>
    ...
    <el-table
      ...
      size="default"
      ref="listTableRef"
      @row-click="listTableToggleSelection"
            ...
            <el-dropdown split-button type="primary" size="small" @click="modifyConfig(scope.row)">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item @click="refreshColumn(scope.row)">刷新表结构</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
...
const refreshColumn = async (config: CodeGenGetOutput) => {
  if (!config) return
  await new CodeGenApi().refreshColumn({ configId: config.id }, { loading: true, showSuccessMessage: true })
}
// 确定
...
const createConfigFromTable = async (table: DbTree) => {
  ...
  var baseFields = ['proId', 'proName', 'proTime', 'modId', 'modName', 'modTime', 'IsDeleted', 'Id']
