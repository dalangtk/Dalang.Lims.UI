  ...
  protected errorHandle(error: any) {
    ...
    if (error.response) {
      switch (error.response.status) {
        ...
        case 500:
          message = '服务器内部错误'
          if (error.response?.data?.msg) message += ',' + error.response.data.msg
          break
        case 501:
          message = '服务未实现'
          break
    ...
    this.instance.interceptors.response.use(
      (res) => {
        ...
        if (res.config?.responseType == 'blob') {
          // if (data.type.includes('application/json') != -1) {
          //   let fileReader = new FileReader()
          //   fileReader.readAsText(data)
          //   fileReader.onload = function (result) {
          //     let jsondata = JSON.parse(result.target!.result!.toString())
          //     if (showErrorMessage) {
          //       ElMessage.error({ message: '提示' + jsondata.msg })
          //     } else {
          //       return Promise.reject(new Error(jsondata))
          //     }
          //   }
          //   return res
          // }
